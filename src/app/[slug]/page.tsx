        ...
        <div className="md:w-2/3 space-y-8 min-h-[500px]">
          ...
          {activeTab === "overview" && (
            <div className="animate-in fade-in slide-in-from-left-4 duration-500 space-y-8">
              <div className="glass rounded-[2rem] p-10 relative overflow-hidden group">
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-brand rounded-full blur-[120px] opacity-10"></div>
                <h2 className="text-3xl font-black text-foreground mb-6 uppercase tracking-tight italic">Sunucu <span className="text-brand">Vizyonu</span></h2>
                <p className="text-muted leading-relaxed text-lg font-medium italic">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="glass rounded-[2rem] p-8 border border-white/5 hover:border-brand/30 transition-all group">
                  <div className="w-12 h-12 bg-brand rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg shadow-brand/20 group-hover:scale-110 transition-transform">⚙️</div>
                  <h3 className="text-xl font-black text-foreground mb-4 uppercase italic">Teknik <span className="text-brand">Güç</span></h3>
                  <ul className="space-y-3 text-muted text-sm font-black uppercase italic">
                    <li className="flex justify-between items-center"><span className="text-muted/50 tracking-widest uppercase text-[10px]">CPU</span> Ryzen 9 7950X</li>
                    <li className="flex justify-between items-center"><span className="text-muted/50 tracking-widest uppercase text-[10px]">RAM</span> 128 GB DDR5</li>
                    <li className="flex justify-between items-center"><span className="text-muted/50 tracking-widest uppercase text-[10px]">DISK</span> 2TB NVMe SSD</li>
                    <li className="flex justify-between items-center"><span className="text-muted/50 tracking-widest uppercase text-[10px]">LINE</span> 10 Gbps</li>
                  </ul>
                </div>
                <div className="glass rounded-[2rem] p-8 border border-white/5 hover:border-purple-500/30 transition-all group">
                  ...
                  <h3 className="text-xl font-black text-foreground mb-4 uppercase italic">Hızlı <span className="text-purple-500">Linkler</span></h3>
                  <div className="flex flex-col gap-4">
                    <Link href={`/${gameSlug}/store`} className="flex items-center justify-between p-3 bg-foreground/5 rounded-xl hover:bg-foreground/10 transition-colors group">
                       <span className="text-[10px] font-black uppercase tracking-widest text-muted">Mağaza</span>
                       <span className="text-brand group-hover:translate-x-1 transition-transform">→</span>
                    </Link>
                    <Link href={`/${gameSlug}/rules`} className="flex items-center justify-between p-3 bg-foreground/5 rounded-xl hover:bg-foreground/10 transition-colors group">
                       <span className="text-[10px] font-black uppercase tracking-widest text-muted">Adalet</span>
                       <span className="text-brand group-hover:translate-x-1 transition-transform">→</span>
                    </Link>
                  </div>
                </div>
          ...
          {activeTab === "leaderboard" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-6">
               <h2 className="text-3xl font-black text-foreground uppercase tracking-tight mb-8 italic">Efsane <span className="text-brand">Savaşçılar</span></h2>
                   ...
                   <tbody className="divide-y divide-white/5">
                     {leaderboard.map((player, idx) => (
                       <tr key={idx} className="hover:bg-foreground/5 transition-colors group">
                         <td className="px-8 py-5 text-xl font-black text-muted/30 italic">#{idx+1}</td>
                         <td className="px-8 py-5">
                            <div className="flex items-center gap-4">
                               <img src={player.head} alt="" className="w-10 h-10 rounded-lg shadow-lg group-hover:scale-110 transition-transform"/>
                               <span className="text-foreground font-black group-hover:text-brand transition-colors uppercase italic">{player.name}</span>
                            </div>
                         </td>
                         ...
                         <td className="px-8 py-5 font-black text-brand italic">{player.kd}</td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
          ...
          {activeTab === "clans" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 grid grid-cols-1 gap-6">
                <h2 className="text-3xl font-black text-foreground uppercase tracking-tight mb-4 italic">Güçlü <span className="text-brand">Klanlar</span></h2>
                {clans.map((clan, idx) => (
                  <div key={idx} className="glass rounded-[2rem] p-8 flex items-center justify-between border border-white/5 hover:border-brand/30 transition-all group">
                    <div className="flex items-center gap-8">
                       <div className="w-16 h-16 bg-foreground/5 rounded-2xl flex items-center justify-center text-3xl shadow-xl border border-white/5">🛡️</div>
                       <div>
                          <h3 className={`text-2xl font-black italic uppercase tracking-tighter ${clan.color}`}>{clan.name}</h3>
                          <p className="text-muted font-black text-[10px] uppercase tracking-widest mt-1 italic">Lider: <span className="text-foreground">{clan.leader}</span></p>
                       </div>
                    </div>
                    <div className="flex gap-10">
                       <div className="text-center">
                          <span className="text-[10px] font-black text-muted uppercase tracking-widest block mb-1">Seviye</span>
                          <span className="text-xl font-black text-foreground italic">{clan.level}</span>
                       </div>
                       <div className="text-center">
                          <span className="text-[10px] font-black text-muted uppercase tracking-widest block mb-1">Üye</span>
                          <span className="text-xl font-black text-foreground italic">{clan.members}</span>
                       </div>
                    </div>
                  </div>
                ))}
